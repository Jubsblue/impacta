...
def read_metadado(meta_path):
    '''
        FUNCÃO: Função de leitura de csv com os parâmetros referentes a transformação dos dados
        INPUT: Path do arquivo de configuração .csv
        OUTPUT: dicionário com as definições do pipeline
    '''
...
def keys_check(df, cols_chaves):
    '''
    Função A função keys_check realiza a verificação das chaves do dataset.
...
def padroniza_str(obs):
    '''
        FUNCÃO: Função que utiliza regex para padronizar os valores
        INPUT: String
...
def classifica_hora(hra):
     '''
        FUNCÃO: Função que define á partir de um horário qual período do dia ele se refere
        INPUT: Horário
        OUTPUT: String com o período do dia (MADRUGADA/MANHA/TARDE)
    '''
...
def flg_status(atraso):
    '''
        FUNCÃO: Função que define á partir do tempo de atraso do voo se ela está atrasado ou dentro do esperado
        INPUT: Tempo de atraso
        OUTPUT: String com o período com flag (ATRASO/ONTIME)
    '''
